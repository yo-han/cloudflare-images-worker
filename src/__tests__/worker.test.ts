...
describe('handleImageRequest', () => {
  ...
  it('serves a sized URL from its own named variant and cache key', async () => {
    ...
    const response = await handleImageRequest(
      new Request('https://worker.dev/mountain-view-1001-200x150.jpeg'),
      mockEnv
    );

    expect(response.status).toBe(200);
    expect(await response.text()).toBe(VARIANT_BYTES);
    expect(fetchedUrls()).toEqual([variantUrl('mountain-view-1001', '200x150')]);
    expect(r2Keys('get')).toEqual([`${CACHE_KEY_PREFIX}/mountain-view-1001/200x150`]);
    expect(r2Keys('put')).toEqual([`${CACHE_KEY_PREFIX}/mountain-view-1001/200x150`]);
  });

  // A size-less URL used to serve the uploaded bytes, EXIF and GPS included.
  // It must only ever serve a metadata-stripped variant.
  describe('never serves original bytes', () => {
    it('serves a size-less URL from the stripped 2048x0 variant', async () => {
      routeFetch({ variantOk: true });

      const response = await handleImageRequest(
        new Request('https://worker.dev/mountain-view-1001.jpeg'),
        mockEnv
      );

      expect(response.status).toBe(200);
      expect(await response.text()).toBe(VARIANT_BYTES);
      expect(fetchedUrls()).toEqual([variantUrl('mountain-view-1001', '2048x0')]);
      expect(r2Keys('get')).toEqual([`${CACHE_KEY_PREFIX}/mountain-view-1001/2048x0`]);
      expect(r2Keys('put')).toEqual([`${CACHE_KEY_PREFIX}/mountain-view-1001/2048x0`]);
    });
    ...
    it.each([
      { path: '/mountain-view-1001.jpeg', upload: false, uploadOk: false },
      { path: '/mountain-view-1001-400x300.jpeg', upload: false, uploadOk: false },
      { path: '/mountain-view-1001.jpeg', upload: true, uploadOk: true },
      { path: '/mountain-view-1001-400x300.jpeg', upload: true, uploadOk: true },
      { path: '/mountain-view-1001.jpeg', upload: true, uploadOk: false },
    ...
    it.each([
      ['/mountain-view-1001.jpeg', 'https://mock-public.com/mountain-view-1001.jpeg'],
      ['/mountain-view-1001-400x300.jpeg', 'https://mock-public.com/mountain-view-1001-400x300.jpeg'],
  ...
  // #151: image names keep the extension exactly as uploaded (`JPG`, `HEIC`,
  // `heif`, `jfif`), and URLs are built from them as-is. Raw and archive
  // formats (dng/ARW/zip) are not servable images.
  describe('extension check', () => {
    it.each(['JPG', 'JPEG', 'PNG', 'HEIC', 'heic', 'heif', 'jfif'])(
      ...
      async (ext) => {
        ...
        const response = await handleImageRequest(
          new Request(`https://worker.dev/forest-trail-1002-200x150.${ext}`),
          mockEnv
        );

        expect(response.status).toBe(200);
        expect(await response.text()).toBe(VARIANT_BYTES);
        expect(fetchedUrls()).toEqual([variantUrl('forest-trail-1002', '200x150')]);
      }
    );
    ...
    it('keeps the extension case in the source URL and the redirect', async () => {
      // The source origin may be case-sensitive: `foo.JPG` is not `foo.jpg` there.
      routeFetch({ variantOk: false, uploadOk: true });

      const response = await handleImageRequest(
        new Request('https://worker.dev/forest-trail-1002-200x150.JPG'),
        mockEnv
      );
      ...
      expect((uploadCall?.[1]?.body as FormData).get('url')).toBe(
        'https://mock-source.com/forest-trail-1002.JPG'
      );
      expect(response.status).toBe(302);
